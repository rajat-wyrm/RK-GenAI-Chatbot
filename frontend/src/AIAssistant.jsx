...
const AIAssistant = () => {
  ...
  const [sessionId, setSessionId] = useState(() => {
    ...
    let sid = window.localStorage.getItem("rk_chat_session");
    if (!sid) {
      sid = (crypto?.randomUUID?.() || `s-${Date.now()}-${Math.random().toString(36).slice(2)}`);
      window.localStorage.setItem("rk_chat_session", sid);
    }
    return sid;
  });
  ...
  const aiCount = useMemo(
    () => messages.filter(message => message.type === "ai" && message.text).length,
    [messages]
  );
  ...
  const handleSend = async text => {
    ...
    setMessages(existing => [
      ...existing,
      { type: "ai", text: "", time: formatTime(new Date()), streaming: true },
    ]);

    try {
      const stream = await streamAiChatMessage(message, {
        sessionId,
        onSources: sources => setActiveSources(sources || []),
        onToken: token => {
          setMessages(existing => {
            const next = [...existing];
            for (let i = next.length - 1; i >= 0; i--) {
              if (next[i].type === "ai" && next[i].streaming) {
        ...
        onDone: payload => {
          if (payload?.session_id) {
            ...
            if (typeof window !== "undefined") {
              window.localStorage.setItem("rk_chat_session", payload.session_id);
            }
          }
        },
        onError: err => {
          setActionError(err.message || "Stream error");
          setMessages(existing => {
            const next = [...existing];
            for (let i = next.length - 1; i >= 0; i--) {
              if (next[i].type === "ai" && next[i].streaming) {
                next[i] = {
                  ...next[i],
                  text: next[i].text || (err.message || "The assistant could not respond right now."),
                  streaming: false,
                  error: true,
                };
                break;
      ...
      setActionError(error.message || "The assistant could not respond right now.");
      setMessages(existing => {
        const next = [...existing];
        for (let i = next.length - 1; i >= 0; i--) {
          if (next[i].type === "ai" && next[i].streaming) {
            next[i] = {
              ...next[i],
              text: next[i].text || (error.message || "The assistant could not respond right now."),
              streaming: false,
              error: true,
            };
            break;
